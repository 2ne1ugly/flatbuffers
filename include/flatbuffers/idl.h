...
#define FLATBUFFERS_GEN_TYPES_SCALAR(TD) \
  TD(NONE,   "",       uint8_t,  byte,   byte,    byte,   uint8,   u8,   UByte,   UInt8,   Byte) \
  TD(UTYPE,  "",       uint8_t,  byte,   byte,    byte,   uint8,   u8,   UByte,   UInt8,   Byte) /* begin scalar/int */ \
  TD(BOOL,   "bool",   uint8_t,  boolean,bool,    bool,   bool,    bool, Boolean, Bool,    Boolean) \
  TD(CHAR,   "byte",   int8_t,   byte,   int8,    sbyte,  int8,    i8,   Byte,    Int8,    Byte) \
  TD(UCHAR,  "ubyte",  uint8_t,  byte,   byte,    byte,   uint8,   u8,   UByte,   UInt8,   Byte) \
  TD(SHORT,  "short",  int16_t,  short,  int16,   short,  int16,   i16,  Short,   Int16,   Short) \
  TD(USHORT, "ushort", uint16_t, short,  uint16,  ushort, uint16,  u16,  UShort,  UInt16,  Short) \
  TD(INT,    "int",    int32_t,  int,    int32,   int,    int32,   i32,  Int,     Int32,   Int) \
  TD(UINT,   "uint",   uint32_t, int,    uint32,  uint,   uint32,  u32,  UInt,    UInt32,  Int) \
  TD(LONG,   "long",   int64_t,  long,   int64,   long,   int64,   i64,  Long,    Int64,   Long) \
  TD(ULONG,  "ulong",  uint64_t, long,   uint64,  ulong,  uint64,  u64,  ULong,   UInt64,  Long) /* end int */ \
  TD(FLOAT,  "float",  float,    float,  float32, float,  float32, f32,  Float,   Float32, Float) /* begin float */ \
  TD(DOUBLE, "double", double,   double, float64, double, float64, f64,  Double,  Double,  Double) /* end float/scalar */
#define FLATBUFFERS_GEN_TYPES_POINTER(TD) \
  TD(STRING, "string", Offset<void>, int, int, StringOffset, int, unused, Int, Offset<String>,  Int) \
  TD(VECTOR, "",       Offset<void>, int, int, VectorOffset, int, unused, Int, Offset<UOffset>, Int) \
  TD(STRUCT, "",       Offset<void>, int, int, int,          int, unused, Int, Offset<UOffset>, Int) \
  TD(UNION,  "",       Offset<void>, int, int, int,          int, unused, Int, Offset<UOffset>, Int)
#define FLATBUFFERS_GEN_TYPE_ARRAY(TD) \
  TD(ARRAY,  "",       int,          int, int, int,          int, unused, Int, Offset<UOffset>, Int)
// The fields are:
// - enum
...
// Generate Scala files from the definitions in the Parser object.
// See idl_gen_scala.cpp
extern bool GenerateScala(const Parser &parser, const std::string &path,
                          const std::string &file_name);
